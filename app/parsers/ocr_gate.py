...
MIN_OCR_CHAR_COUNT = 200
MIN_OCR_WORD_COUNT = 30


def ocr_text_is_usable(char_count: int, word_count: int) -> bool:
    """Return True when OCR output clears the minimum usable thresholds."""
    return char_count >= MIN_OCR_CHAR_COUNT and word_count >= MIN_OCR_WORD_COUNT
...
def ocr_quality(parse_result: dict | None) -> tuple[str, str]:
    ...
    if ocr_applied and char_count < MIN_OCR_CHAR_COUNT:
        return "blocked", (
            f"OCR ran but recovered only {char_count} characters ({word_count} words) - "
            f"below the minimum threshold ({MIN_OCR_CHAR_COUNT} chars / {MIN_OCR_WORD_COUNT} words). "
            "The scan quality is too low for reliable extraction. "
            "Provide a higher-resolution scan or a text-layer PDF."
        )

    if ocr_applied and word_count < MIN_OCR_WORD_COUNT:
        return "blocked", (
            f"OCR ran but recovered only {word_count} words - "
            f"below the minimum threshold ({MIN_OCR_WORD_COUNT} words). "
